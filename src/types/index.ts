...
export interface User {
  ...
  /** 1日の目標問数（自分で設定・ストリーク判定にも使う） */
  dailyGoal: number
  /** 「今日の一式」に表示する項目のID（順序も保持）。login は自動で常設 */
  dailyTasks: string[]
  /** 起動型タスク(listening/phrase/example/review)を今日実行したか。taskId→YYYY-MM-DD */
...
export interface PetState {
  ...
  formSeen: number
  /** 合体の段階(0=通常, 1..5)。上がるほど見た目が豪華になる */
  fusion?: number
  /** 入手直後の「NEW」表示用。育成中にする等で消える */
  isNew?: boolean
}
