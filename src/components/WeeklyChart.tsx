...
export function WeeklyChart() {
  const { user } = useGame()
  const { t } = useNav()
  const dayLabels = t('weekly.days')
  const days: { key: string; label: string; count: number }[] = []
  for (let i = 6; i >= 0; i--) {
    const d = new Date()
    d.setDate(d.getDate() - i)
    const key = todayStr(d)
    days.push({ key, label: dayLabels[d.getDay()], count: user.dailyHistory[key] ?? 0 })
  }
  const max = Math.max(streakConfig.dailyGoal, ...days.map((d) => d.count))
  return (
    <div className="card p-4">
      <div className="text-xs text-white/45 font-bold mb-2">{t('weekly.thisWeek')}</div>
